...
__export(lib_exports, {
  createPageResolver: () => createPageResolver,
  resumable: () => resumable
});
...
// lib/plugin/combine-sourcemaps.ts
var nullSourceMap = {
  names: [],
  sources: [],
  mappings: "",
...
function _getCombinedSourcemap(filename, sourcemapChain) {
...
// lib/transform/ast/var-from-patterns.ts
function variableFromPatterns(patterns) {
  const vars = [];
...
// lib/transform/scope.ts
var Scope3 = class {
  constructor(rootStart, owner, thisable, parent) {
    this.rootStart = rootStart;
    this.owner = owner;
    this.thisable = thisable;
    this.parent = parent;
    this.declarations = /* @__PURE__ */ new Map();
    this.references = [];
    this.closures = [];
    this.children = [];
    this.expressions = /* @__PURE__ */ new Map();
    if (parent) {
      parent.children.push(this);
    }
  }
  create(rootStart, node, thisable) {
    return new Scope3(rootStart, node, thisable, this);
  }
  mergeChildren() {
    for (const child of this.children) {
      for (const ref2 of child.references) {
        if (ref2 instanceof Closure) {
          this.references.push(ref2);
        } else if (ref2.type === "Identifier") {
          if (!child.declarations.has(ref2.name)) {
            let scope = this;
            while (scope) {
              if (scope.declarations.has(ref2.name)) {
                this.references.push(ref2);
                break;
              }
              scope = scope.parent;
        ...
        } else if (ref2.type === "ThisExpression") {
          if (!child.thisable) {
            this.references.push(ref2);
          }
        }
...
var Closure = class {
  constructor(exportName, parent, scope, entry) {
...
function parse4(code) {
  ...
  const imports = [];
  const programScope = new Scope3(0, ast, false);
  const scopes = [programScope];
  let entryDeclaration = null;
  let rootStart = 0;
  const skipEnter = /* @__PURE__ */ new Map();
  walk(ast, {
    enter(n2, p) {
      const node = n2;
      const parent = p || ast;
      ...
      if (skipEnter.has(node)) {
        ...
        for (const [v, p2] of variableFromPatterns(node.params)) {
          funcScope.declarations.set(v, p2);
        }
        scopes.push(funcScope);
        for (const p2 of node.params)
          skipEnter.set(p2, "deep");
        ...
        skipEnter.set(node.id, "deep");
        const vars = [];
        for (const [v, p2] of variableFromPatterns([node.id])) {
          vars.push(v);
          scope.declarations.set(v, p2);
        }
  ...
  return [programScope, ast, imports];
}
function __alias(str, { id, start, end }, exportIndex) {
...
var Import = class {
  ...
  update(magicString, target) {
    const { decl } = this;
    const source = decl.source;
    if (!CSS_LANGS_RE.test(source.value)) {
      if (source) {
...
function selectRootClosures(root, hasClosure) {
  const stack = [root];
  const retval = [];
...
function selectClosures(rootScope, entries) {
  ...
  while (stack.length) {
    ...
    if (entries instanceof Array) {
      for (const cl of scope.closures) {
        if (entries.includes(cl.exportName))
          retval.push(cl);
      }
    } else {
...
function formattedArgs(closure, hasClosure) {
...
function getBindings(closure, hasClosure, stack = /* @__PURE__ */ new Set()) {
  ...
  for (const ref2 of closure.scope.references) {
    if (ref2 instanceof Closure) {
      if (hasClosure(ref2)) {
      }
    } else if (ref2.type === "Identifier") {
      if (!closure.scope.declarations.has(ref2.name) && resolve3(closure.scope.parent, ref2.name)) {
        bindings.set(ref2.name, ref2.name);
      }
  ...
  return bindings;
  function resolve3(leaf, ref2) {
    let scope = leaf;
    while (scope) {
      if (scope.declarations.has(ref2))
        return true;
      scope = scope.parent;
    }
    return false;
...
function formatBindings(bindings, stack = /* @__PURE__ */ new Set()) {
  ...
  return [params, args.join(", "), deps];
}

// lib/transform/server.ts
var CLOSURE_HELPER = `
...
`;
function transformServer(code, opts) {
  const [rootScope, program, imports] = parse4(code);
  const magicString = new MagicString(code);
  const closures = selectClosures(rootScope, null);
  function hasClosure(cl) {
    return closures.includes(cl);
  }
  for (const closure of closures) {
    exportClosure(magicString, closure, hasClosure);
  }
  if (opts.includeHelper === void 0 || opts.includeHelper === true) {
...
function exportClosure(magicString, closure, hasClosure) {
  const { scope, parent } = closure;
  if (scope.owner.type === "FunctionDeclaration" || scope.owner.type === "ClassDeclaration") {
...
// lib/transform/client.ts
function transformClient(code, opts) {
  const [rootScope, program, imports] = parse4(code);
  const magicString = new MagicString(code);
  function hasClosure(cl) {
  ...
  const rootClosures = selectRootClosures(rootScope, hasClosure);
  const stack = [];
  for (const closure of rootClosures) {
    exportClosure2(magicString, closure, hasClosure);
...
function exportClosure2(magicString, closure, hasClosure) {
...
function updateClosureReferences(magicString, closure, hasClosure) {
  ...
  if (subOwner.type === "FunctionDeclaration" || subOwner.type === "ClassDeclaration") {
    magicString.appendLeft(
      closure.scope.owner.start,
      `function ${subOwner.id.name}(...args) { return ${closure.exportName}(${subArgs})(...args) }`
    );
  } else {
      ...
      switch (subParent.type) {
        case "MethodDefinition":
          if (subOwner.type === "FunctionExpression") {
            magicString.appendRight(
              subOwner.start,
              ` /* ${subParent.key.type === "Identifier" ? subParent.key.name : ""} */ ${subOwner.async ? "async" : ""} function `
            );
          }
          magicString.appendLeft(
            closure.scope.owner.start,
...
function stripProgram(magicString, program, excludes) {
  ...
  walk(program, {
    enter(n2) {
      ...
      if (node.type === "ImportDeclaration") {
        this.skip();
        start = n2.end;
      } else if (excludes.includes(n2)) {
        magicString.appendLeft(start, "\n");
        magicString.remove(start, n2.start);
        start = n2.end;
        this.skip();
      }
    }
...
function fileToUrl(file, root) {
  return "/" + import_node_path.default.relative(root, file).replace(/\\/g, "/");
}

// lib/plugin/page-loader.ts
var CSS_LANGS_RE2 = /\.(css|less|sass|scss|styl|stylus|pcss|postcss|sss)(?:$|\?)/;
var RESUMABLE_URL_RE = /\/@(client(\[([^\)]+)\])?|server)\//;
...
var fnDeclarationLineCount = 0;
{
  const body = "/*code*/";
  const source = new AsyncFunction("a", "b", body).toString();
  fnDeclarationLineCount = source.slice(0, source.indexOf(body)).split("\n").length - 1;
}
function loadModule(vite, load) {
  const loadCache = /* @__PURE__ */ new Map();
  const moduleCache = /* @__PURE__ */ new Map();
  const loadstack = [];
  function moduleLoader(url) {
...
function createLoader(server) {
  const loadResumableModule = loadModule(
    server,
    async (url, target, ssrModule) => {
      ...
      const ssrDynamicImport = (dep) => {
        if (dep[0] === ".") {
          dep = import_node_path2.default.posix.resolve(import_node_path2.default.dirname(url), dep);
        }
      ...
      let sourceMapSuffix = "";
      const map = _getCombinedSourcemap(filename, sourcemapChain);
      if (map) {
...
//# sourceURL=${filename}${sourceMapSuffix}`
  ...
  async function loadAndTransform(url, target = "server", entries = null) {
    let source = url;
    ...
    if (baseResult.map) {
      const targettedMap = {
        ...baseResult.map,
        sources: baseResult.map.sources.map(
          (x) => fileToUrl(x, server.config.root)
        )
      };
      sourcemapChain.push(targettedMap);
    }
    const resumeResult = target === "server" ? transformServer(baseResult.code, {}) : transformClient(baseResult.code, {
      entries
    });
    if (!resumeResult)
      return null;
...
function parseResumableUrl(url) {
  ...
  return null;
  function createModuleUrl(match) {
    const resMatchIndex = match.index || 0;
    const resMatchLength = match[0].length || 0;
    return url.substring(0, resMatchIndex) + "/" + url.substring(resMatchIndex + resMatchLength);
  }
}

// lib/plugin/file-route-resolver.ts
var FileRouteResolver = class {
  constructor(baseDir) {
    this.baseDir = baseDir;
    this.resolvePage = (path4) => {
      if (path4 === null || path4 === void 0)
        return null;
      const scriptPath = this.baseDir + (path4.endsWith("/") ? path4 + "index" : path4 || "/index") + ".tsx";
      return scriptPath;
    };
  }
};

// lib/hibernate/utils.ts
var RefMap = class {
  constructor() {
...
function contentHash(baseName, str, range, fill = "_".charCodeAt(0)) {
...
var primitives = ["number", "bigint", "boolean"];

// lib/hibernate/writer.ts
var HibernationWriter = class {
  ...
  async write(value) {
    ...
    while (stack.length) {
      ...
      if (curr === null || curr === void 0) {
        ...
        const { __src, __name, __args } = curr;
        if (__src && __name) {
...
function ${loader}(){ return import("${source}") }`
  ...
  importDesc(value) {
    ...
    const { __src, __name, __args } = value;
    if (!__src || !__name) {
  ...
  return {
    ...
    config() {
      return {
        resolve: {
          alias: {
            "@resumable": import_node_path3.default.resolve(__dirname)
          }
        }
      };
    },
    configureServer(vite) {
      function createDefaultPageResolver() {
        console.log(
          "Resumable scripts will be resolved from: " + kleur_default.gray(vite.config.root) + kleur_default.green("/pages")
        );
    ...
    async resolveId(source) {
      ...
      if (source && source.startsWith(prefix)) {
        const resolved = import_node_path3.default.resolve(
          __dirname,
